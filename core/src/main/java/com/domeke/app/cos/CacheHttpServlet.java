...
class CacheHttpServletResponse implements HttpServletResponse {
	...
	private int status;
	private Hashtable<Object, Vector<Object>> headers;
	private String contentType;
	private String encoding;
	private Locale locale;
	private Vector<Object> cookies;
	...
	private void internalReset() {
		status = 200;
		headers = new Hashtable<Object, Vector<Object>>();
		contentType = null;
		encoding = null;
		locale = null;
		cookies = new Vector<Object>();
		didError = false;
		didRedirect = false;
		gotStream = false;
		gotWriter = false;
	...
	private void internalAddHeader(String name, Object value) {
		Vector<Object> v = (Vector<Object>) headers.get(name);
		if (v == null) {
			v = new Vector<Object>();
		}
		v.addElement(value);
	...
	public void writeTo(HttpServletResponse res) {
		...
		if (encoding != null)
			((CacheHttpServletResponse) res).setCharacterEncoding(encoding);
		if (locale != null)
			res.setLocale(locale);
		// Write cookies
		Enumeration<Object> enu = cookies.elements();
		...
		while (enu.hasMoreElements()) {
			...
			Vector<Object> values = (Vector<Object>) headers.get(name); // may
																		// have
																		// multiple
			// values
			Enumeration<Object> enu2 = values.elements();
	...
	public void setCharacterEncoding(String encoding) {
		((CacheHttpServletResponse) delegate).setCharacterEncoding(encoding);
		encoding = encoding;
	}
	...
	public String getContentType() {
		return ((CacheHttpServletResponse) delegate).getContentType();
	}
